...
echo "Creating VMs"
create_vm pytorch-1 tubify-438815 n1-standard-2 $sa $im

# check if the VMs are created
echo "Checking if VMs are created"
retry_command check_vm_creation pytorch-1

# Setup the kubeconfig file
ip=$(get_instance_ip pytorch-1)
echo "ssh -i ~/.ssh/$ssh_file shivkb@$ip -L 2222:localhost:8888"
